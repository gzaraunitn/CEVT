...
def main(trial):
    ...
    parser.add_argument(
        "--open_method",
        default="cls EVT",
        choices=["OSVM", "cls OSVM", "EVT", "cls EVT"],
    )
    parser.add_argument(
        "--OSVM_threshold", default=0.995
    )  # args.OSVM_threshold python main.py
    ...
    # args.class_file = "data/classInd_ek.txt"
    ...
    if method == "CEVT":
        ...
        args.dis_DA = method
        args.alpha = (
            trial.suggest_uniform("alpha", 0.01, 2) if args.auto else 0.1
        )  # 1 | 0.1 | 0.01
        args.OSVM_threshold = (
            trial.suggest_uniform("OSVM_threshold", 0.5, 0.99) if args.auto else 0.99
        )
    elif method == "MCD" or method == "AdaBN":
        args.ens_DA = method if method == "MCD" else "none"
        args.use_bn = method if method == "AdaBN" else "none"
        args.OSVM_threshold = (
            trial.suggest_uniform("OSVM_threshold", 0.5, 0.99) if args.auto else 0.9999
        )
    ...
    if args.auto == False:
        ...
        if args.use_target == "none":
            ...
            if args.use_bn != "none":
                print(
                    Fore.GREEN + "Apply the adaptive normalization approach:",
                    args.use_bn,
                )
    ...
    filtered_parameters = [p for p in model.parameters() if id(p) not in resnet_p_ids]
    ...
    class_id_list = [
        int(line.strip().split(" ")[2]) for line in open(args.train_source_list)
    ]
    ...
    val_set = TSNDataSet(
        ...
        if args.modality in ["RGB", "RGBDiff", "RGBDiff2", "RGBDiffplus"]
    ...
    if args.evaluate:
        ...
        prec1, entropy_cuts = validate(
            val_loader, model, criterion, num_class, 0, test_file, meter
        )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % args.eval_freq == 0 or epoch == args.epochs:
            prec1, entropy_cuts = validate(
                val_loader, model, criterion, num_class, epoch, val_file, meter
            )
...
def train(
    ...
    for i, ((source_data, source_label), (target_data, target_label)) in data_loader:
        ...
        # source_label, target_label = torch.tensor(
        #     [int(i) for i in source_record[2]]
        # ), torch.FloatTensor([int(i) for i in target_record[2]])
        ...
        if (
            ...
            for l in range(len(args.place_adv)):
                if args.place_adv[l] == "Y":
                    ...
                    loss_adversarial_single_source = criterion_domain(
                        pred_domain_source_single,
                        source_domain_label.cuda(non_blocking=True),
                    )

                    # ------- Computing Target Advserial Weights -------#
                    target_entropy = -(
                        F.softmax(out_target, dim=1) * F.log_softmax(out_target, dim=1)
                    ).sum(dim=1)
                    ...
                    entropy_max = -(
                        F.softmax(even_tensor, dim=0)
                        * F.log_softmax(even_tensor, dim=0)
                    ).sum()
                    ...
                    for i_ in range(list(out_target.shape)[0]):
                        ...
                        weight = (
                            0.5
                            + args.mix_interval_length
                            * (entropy_cut - float(target_entropy[i_]))
                            / hard_interval_half
                        )  # 0.25 0.75 1
                    ...
                    if (
                        list(weights.shape)[0]
                        != list(pred_domain_target_single.shape)[0]
                    ):
                        weights = tile(weights, 0, args.num_segments)

                    loss_adversarial_single_target = ivan_CEL(
                        pred_domain_target_single,
                        target_domain_label.cuda(non_blocking=True),
                        weights.cuda(),
                    )
                    # loss_adversarial += loss_adversarial_single
...
def validate(val_loader, model, criterion, num_class, epoch, log, meter):
    ...
    for i, (val_data, val_label) in enumerate(val_loader):
        ...
        if batch_val_ori < args.batch_size[2]:
            val_data_dummy = torch.zeros(
                args.batch_size[2] - batch_val_ori,
                val_size_ori[1],
                val_size_ori[2],
                val_size_ori[3],
                val_size_ori[4],
        ...
        with torch.no_grad():
            ...
            if args.open_method == "EVT":  # EVT
                ...
                pred1 = (
                    torch.cat((pred1, pred.data.max(1)[1]))
                    if i != 0
                    else pred.data.max(1)[1]
                )

                # loss = criterion(pred, label)
                target_entropy = -(
                    F.softmax(pred, dim=1) * F.log_softmax(pred, dim=1)
                ).sum(dim=1)
                ...
                label[
                    label > num_class
                ] = num_class  # change all extra classes in target to unk
                ...
                for cls in range(num_class):
                    ...
                    if logits_per_cls.nelement() != 0:
                        entropy_per_cls = -(
                            F.softmax(logits_per_cls, dim=1)
                            * F.log_softmax(logits_per_cls, dim=1)
                        ).sum(dim=1)
                ...
                if len(target_entropy_all) == len(val_loader.dataset):
                    ...
                    for cls in range(num_class):
                        if len(target_entropy_cls[cls]) != 0:
                            ...
                            entropy_cuts.append(
                                100
                            )  # no entropy can be greater than 100
                ...
                for cls in range(num_class):
                    ...
                    if int(list(logits_per_cls.shape)[0] * threshold_value) != 0:
                        threshold = (
                            logits_per_cls.topk(
                                int(list(logits_per_cls.shape)[0] * threshold_value)
                            )[0]
                            .cpu()
                            .numpy()[-1]
                        )
                ...
                idx = (
                    pred[:, :].max(-1)[0].detach().double()
                    < class_threshold[pred[:, :].max(-1)[1]].double()
                )
    ...
    if args.auto == False:
        ...
        print(
            "ALL: {:.4f},  OS: {:.4f}, OS*: {:.4f}, UNK: {:.4f}, "
            "balanced: {:.4f}.".format(
                meter.sum.sum() / meter.count.sum(),
                meter.avg.mean(),
                meter.avg[:-1].mean(),
                meter.avg[-1],
                2
                * (meter.avg[-1] * meter.avg[:-1].mean())
                / (meter.avg[:-1].mean() + meter.avg[-1]),
            )
        )

        wandb.log(
            {
                "val_acc": meter.sum.sum() / meter.count.sum(),
                "closed_acc": meter.avg[:-1].mean(),
                "open_acc": meter.avg[-1],
                "h_score": 2
                * (meter.avg[-1] * meter.avg[:-1].mean())
                / (meter.avg[:-1].mean() + meter.avg[-1]),
            }
